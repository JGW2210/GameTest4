...
(function (root, factory) {
  ...
  const ENEMIES = {
    ...
    sprigwitch: { world: 'woods', name: 'Sprig Witch',      icon: '🧙', hp: 28, pattern: [VENOM(3, 3), A(6), [A(7), DBF(0, 1)]] },
    owlsage:    { world: 'woods', name: 'Heretic Owl-Sage', icon: '🦉', hp: 34, elite: true, pattern: [LEECH(2, 4), A(8), BUFF(2), A(9)] },
    briarhulk:  { world: 'woods', name: 'Briar Hulk',       icon: '🌳', hp: 42, elite: true, pattern: [B(8), A(11), A(6, 2)] },
    grovemaw:   { world: 'woods', name: 'The Grove-Maw',    icon: '🍃', hp: 72, boss: true,
                  pattern: [A(9), VENOM(4, 4), B(9), A(7, 2), BUFF(2)] },

    /* ---------- Tier 2: Sunken Catacombs ---------- */
    drownedscribe:{ world: 'catacombs', name: 'Drowned Scribe', icon: '🫧', hp: 27, pattern: [A(7), LEECH(1, 5), A(8)] },
    ...
    rustgolem:  { world: 'catacombs', name: 'Rust Golem',       icon: '🗿', hp: 36, pattern: [B(9), A(11), A(8)] },
    ...
    lexovore:   { world: 'catacombs', name: 'The Lexovore',     icon: '🐍', hp: 84, boss: true,
                  pattern: [LEECH(3, 6), A(11), B(10), A(8, 2), DBF(2, 2)] },

    /* ---------- Tier 2 ALT: Drowned Orchard (venom & attrition) ---------- */
    rotfruit:   { world: 'orchard', name: 'Rotfruit Shambler', icon: '🍎', hp: 29, pattern: [VENOM(4, 4), A(7), VENOM(3, 5)] },
    ...
    wormqueen:  { world: 'orchard', name: 'Worm Queen',        icon: '🪱', hp: 38, elite: true, pattern: [VENOM(5, 4), A(7, 2), BUFF(3), VENOM(7, 0)] },
    grafter:    { world: 'orchard', name: 'THE GRAFTER',       icon: '✂️', hp: 88, boss: true,
                  pattern: [VENOM(5, 5), A(10), HEAL(8), A(7, 2), DBF(2, 1), VENOM(7, 6)] },

    /* ---------- Tier 3: Ashen Peaks ---------- */
    cinderimp:  { world: 'peaks', name: 'Cinder Imp',       icon: '👹', hp: 28, pattern: [SMOLDER(4, 4), A(9), A(7)] },
    slagbeast:  { world: 'peaks', name: 'Slag Beast',       icon: '🐗', hp: 35, pattern: [A(11), B(8), A(9)] },
    ashwidow:   { world: 'peaks', name: 'Ash Widow',        icon: '🕷️', hp: 29, pattern: [VENOM(5, 4), SMOLDER(3, 3), A(8)] },
    pyrecaller: { world: 'peaks', name: 'Pyre Caller',      icon: '🔥', hp: 31, pattern: [BUFF(2), SMOLDER(5, 5), A(10)] },
    magmaTome:  { world: 'peaks', name: 'Magma-Bound Tome', icon: '📕', hp: 44, elite: true, pattern: [SMOLDER(6, 6), A(12), B(9), A(9, 2)] },
    obsidianknight:{ world: 'peaks', name: 'Obsidian Knight', icon: '♞', hp: 50, elite: true, pattern: [B(12), A(14), DBF(1, 1), A(11)] },
    cinderking: { world: 'peaks', name: 'The Cinder King',  icon: '👑', hp: 96, boss: true,
                  pattern: [SMOLDER(6, 6), A(12), BUFF(3), A(9, 2), B(12)] },

    /* ---------- Tier 3 ALT: Glass Desert (mirrors & armor) ---------- */
    glasscrab:  { world: 'glass', name: 'Glass Crab',       icon: '🦀', hp: 30, pattern: [B(10), A(10), B(7)] },
    ...
    prismwyrm:  { world: 'glass', name: 'Prism Wyrm',       icon: '🐛', hp: 46, elite: true, pattern: [B(12), A(13), SMOLDER(5, 5), A(10, 2)] },
    hourtitan:  { world: 'glass', name: 'Sandglass Titan',  icon: '⏳', hp: 52, elite: true, pattern: [B(14), A(15), LEECH(3, 6), BUFF(3)] },
    vitrifier:  { world: 'glass', name: 'THE VITRIFIER',    icon: '🔍', hp: 100, boss: true,
    ...
    hollowedjudge:{ world: 'void', name: 'Hollowed Judge', icon: '⚖️', hp: 38, pattern: [B(10), A(13), DBF(0, 2)] },
    voidlarva:  { world: 'void', name: 'Void Larva',       icon: '🐛', hp: 32, pattern: [VENOM(6, 5), HEAL(5), A(11)] },
    ...
    censor:     { world: 'void', name: 'The Grand Censor', icon: '🚫', hp: 110, boss: true,
                  pattern: [LEECH(4, 8), A(13), B(13), DBF(2, 2), A(10, 2), BUFF(3)] },

    /* ---------- Tier 4 ALT: Moth Court (insight theft & swarms) ---------- */
    courtmoth:  { world: 'mothcourt', name: 'Courtier Moth',   icon: '🦋', hp: 28, pattern: [LEECH(2, 5), A(9, 2), DBF(1, 0)] },
    dustcloak:  { world: 'mothcourt', name: 'Dustcloak',       icon: '🧥', hp: 33, pattern: [A(11), LEECH(1, 6), B(8)] },
    lanternthief:{ world: 'mothcourt', name: 'Lantern Thief',  icon: '🏮', hp: 30, pattern: [LEECH(3, 0), A(12), A(8)] },
    silkjudge:  { world: 'mothcourt', name: 'Silk Judge',      icon: '🎀', hp: 36, pattern: [DBF(2, 1), A(10), B(9), A(12)] },
    ...
    mothqueen:  { world: 'mothcourt', name: 'THE MOTH QUEEN',  icon: '👸', hp: 115, boss: true,
    ...
    cometling:  { world: 'spire', name: 'Cometling',        icon: '☄️', hp: 33, pattern: [A(9, 2), BUFF(2), A(12)] },
    constellation:{ world: 'spire', name: 'Bound Constellation', icon: '✨', hp: 58, elite: true, pattern: [A(11, 2), B(13), BUFF(4), A(16)] },
    ...
    erratabeast:{ world: 'margin', name: 'Errata Beast',     icon: '📝', hp: 36, pattern: [A(14), DBF(2, 0), A(10, 2)] },
    ...
    inkleech:   { world: 'margin', name: 'Ink Leech',        icon: '🩸', hp: 38, pattern: [VENOM(7, 5), HEAL(6), A(12)] },
    redactor:   { world: 'margin', name: 'The Redactor',     icon: '🖊️', hp: 60, elite: true, pattern: [LEECH(4, 8), A(16), B(12), DBF(2, 2)] },
    appendixwyrm:{ world: 'margin', name: 'Appendix Wyrm',   icon: '🐉', hp: 64, elite: true, pattern: [A(12, 2), BUFF(4), A(18), B(11)] },
    marginalia: { world: 'margin', name: 'THE MARGINALIA',   icon: '❦', hp: 138, boss: true,
    ...
    nullscribe: { world: 'unwritten', name: 'Null Scribe',    icon: '🕳️', hp: 48, pattern: [B(14), A(17), LEECH(4, 0), A(12, 2)] },
    unwrittenone:{ world: 'unwritten', name: 'THE UNWRITTEN ONE', icon: '⬛', hp: 170, boss: true,
  ...
  return { WORLDS, WORLD_BY_ID, variantsForTier, ENEMIES, BY_WORLD, SCALING, tierHpMult, tierDmgMult, scaledStats, goldReward };
});
